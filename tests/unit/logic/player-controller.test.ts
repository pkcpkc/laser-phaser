...
describe('PlayerController', () => {
    ...
    it('should return to upright when blocked by boundary', () => {
        // Setup: ship moving towards wall
        (controller as any).setTarget(-100, 100); // Target outside

        // Mock that we are moving (so returnToUpright isn't skipped due to direction='none')
        (controller as any).currentTiltDirection = 'moving';
        (controller as any).targetEffect = { destroy: vi.fn() };

        // Force ship out of bounds
        mockShip.sprite.x = -10;

        controller.update(0);

        // Should trigger returnToUpright -> tweens.add with angle -90
        expect(mockScene.tweens.add).toHaveBeenCalledWith(expect.objectContaining({
            targets: mockShip.sprite,
            angle: -90
        }));
